...
- Wynne Systems publishes a Trackunit integration on its own site (compatible with RentalMan and TMS); Trackunit Marketplace connector listings for RentalMan and Integrated Rental reportedly exist, but the marketplace URLs are login-gated and could not be independently verified. Trackunit also has a formal, publicized partnership with Point of Rental (two-way API with Expert and Elite products) (https://trackunit.com/press/point-of-rental-partnership/).
...
| Wynne Systems (RentalMan, RentalResult) | Volaris-era enterprise rental ERP; RentalMan used by very large rental companies; RentalResult targets contractors and ex-SAP-ETM users; also distributed via InTempo | Trackunit integration page on wynnesystems.com (compatible with RentalMan and TMS); a RentalMan ERP connector reportedly listed in Trackunit Marketplace (login-gated, not independently verified) | Enterprise accounts with big mixed fleets; deepen from asset feed to workflow-level (service, dispatch) integration |
| inspHire | Owned by Kerridge Commercial Systems (acquired 2018); ~10,000 users; offices UK, US, AU, NL | Dedicated Trackunit integration (live location, status, contract info, 7-day usage, meter readings inside inspHire Asset Tracker); named on IrisX integrations page | KCS group-level deal could cover inspHire plus sister products (Current RMS); strong UK/EU hire market reach |
...
| Integrated Rental | US rental software for heavy equipment dealers, ERP/DMS-connected (site now hosted under VitalEdge Technologies) | ERP connector reportedly listed in Trackunit Marketplace (not independently verified; Integrated Rental's own site mentions telematics integration generally but does not name Trackunit) | Dealer rental ops; potential marketplace asset to grow |
...
_Verified: Cross-checked against primary sources. Confirmed directly: IrisX integrations page (1,200+ prebuilt connectors; REST/GraphQL/Time Series/Rental ERP APIs; NetSuite, Dynamics 365, Acumatica, Baseplan, inspHire, MCS named), Trackunit-Point of Rental partnership press release (two-way API, Expert/Elite, Manager/Go, on/off-rent, delivery and billing data), RentalResult SAP ETM sunset article (BearingPoint-cited 2025 usage rights / 2027 maintenance dates), inspHire-Trackunit integration page (live location, status, contract info, 7-day usage, meter readings; KCS ownership; UK/US/AU/NL offices), inspHire ~10,000 users (inspHire/KCS announcement), Point of Rental 5,000+ locations in 80 countries with offices incl. Toronto (company LinkedIn and press releases), MCS Telematics Hub provider list (Trackunit, JCB LiveLink, Enigma, Teletrac, Eroad per MCS news page), Texada-Trackunit partner page (Rental Management, Service Management; Mississauga and Queensland offices), Rouse Rental Insights (400+ companies, $115B fleet value), ISO 15143-3/AEMP 2.0 APIs from Caterpillar, Volvo CE and John Deere (OEM developer pages). Corrected: inspHire acquisition by KCS was 2018, not 2016. Softened: Trackunit Marketplace listings for RentalMan and Integrated Rental connectors could not be independently verified (login-gated marketplace); the Wynne-Trackunit integration itself is confirmed on wynnesystems.com._
