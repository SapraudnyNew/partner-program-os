...
## SESSION 2026-07-04 · partner-abm-module

Fifth section added to the Trackunit branch: Partner ABM (docs/trackunit/abm/), per ADR-013. Three-layer model (ABM-to / ABM-with / ABM-through partner) applied to partner acquisition. Pages: index, targeting (funnel 37 to five on the published landscape scoring), plays (layer playbook + five play cards with named entry threads), procore-showcase (full depth: 7-person committee, 12-week cadence, content matrix, joint ABM, marketplace channel, KPI tree), operating-system (stack-agnostic CRM spec, signal stack, 12-week launch). Interactives on a single data source abm/data.js (6 accounts, 42 people, 28 signals compiled from account plans): control-tower, signal-radar, relationship-heatmap. Six print-ready HTML executive briefs in abm/briefs/. Memo homepage now shows four perspectives; converter nav/sidebar extended; hand-built pages patched. No new people research; no Apify calls; honesty labels (stages = starting-point estimates, credit figures = assumptions) on every surface.
